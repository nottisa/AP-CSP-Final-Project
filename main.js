...
function sortArray(arrayToSort, type) {
    ...
    while (arrayToSort.length > 0) {
        ...
        for (let i2 = 0; i2 < arrayToSort.length; i2++) {
            if (lowestValue == '' || sortJson[type].indexOf(arrayToSort[i2][0]) < sortJson[type].indexOf(lowestValue[0])) { //Mozilla web docs https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Array/indexOf
        ...
        arrayToSort.splice(arrayToSort.indexOf(lowestValue), 1); //Mozilla web docs https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Array/splice
    }
    return finalArray;
}
...
function generateTypeOptions() {
    let toReturn = [];
    for (let [i, i2] of Object.entries(sortJson)){ //Mozilla web docs https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Object/entries
        toReturn.push(i);
    }
    return toReturn;
}
...
function main() {
    ...
    userInput.question('How would you like to sort the above list? ' + generateTypeOptions().join(', ') + " >>> ", input => { //Node.js docs https://nodejs.org/en/learn/command-line/accept-input-from-the-command-line-in-nodejs
